...
Sensors need to be located on every side of the robot, with one sensor on every side. This means there needs to be a sensor on the Front, Right, Rear, and Left of your robot. **The sensors also need to meet the following requirements to work properly**:
 ...
 - **(Optional & Optimal)** Works with metal fields - To have the sensors work with the metal fields, the sensors have to mounted in a way that causes them to hit the metal bar at the top of the field and not the clear polycarbonate walls. There is an low chance to get accurate readings off the clear polycarbonate walls
...
## Measuring Sensor Offsets

To make the sensors accurate, you need to find the **parrallel (X) and perpendicular (Y)** offsets of the sensors of your robot.
...
The perpendicular offset for this sensor would be **negative** since the sensor is past the origin in that direction near the back of the robot 
...
### Clarification of offsets
 ...
 - X (parrallel) offsets are relative to the sensor
 - Y (perpendicular) offset are abosulte to the chassis.

#### Perpendicular offset guide
...
This is consistent with **LemLib's** offsets for odometry wheels.
...
##### Collected Offsets Example (Matches above except for negative example)
 ...
 - West `X: 5.137, Y: -3.744`

## If Using The Legacy Metal Field Perimeter

Contrary to popular belief, The metal field perimeter can actually be used with DSR against the polycarbonate walls but with some very important caveats:

 - The sensor must be relatively square to the field ~10 deg tolerance
 - The sensor must be relatively close to the field < 40 inches
 - If the wall plastic is new, there is a decent change a more opaque object behind it will be read by the sensor
 - If there is suboptimal lighting, the sensor reading will be inconsistent

![alt text](image-22.png)

#### This will work in practice, but with the caveats listed above

## Physical Setup Complete!

You just completed the hard part of TitanReset! Learn how to program your new sensors with TitanReset to improve your autons [here](ProgrammingSetup.MD)

__

### Pro tip - you can change the orientation of the V5 distance sensor module on the base

Since the distance sensor module is seperate to the base and held in with 4 screws in the same pattern, the module itself can be rotated to fit in ways that might not of before seemed possible.

##### Default sensor orientation

![alt text](image-13.png)

##### Modified sensor orientation
